...
def test_allows_creating_new_semver_tag():
    ...
    stdin_data = "refs/tags/v1.0.2 aaaa1111 refs/tags/v1.0.2 0000000000000000000000000000000000000000\n"
    res = subprocess.run(["bash", str(HOOK_PATH), "origin"], input=stdin_data, capture_output=True, text=True, check=False)
...
def test_blocks_deleting_existing_semver_tag():
    ...
    stdin_data = "refs/tags/v1.0.0 0000000000000000000000000000000000000000 refs/tags/v1.0.0 aaaa1111\n"
    res = subprocess.run(["bash", str(HOOK_PATH), "origin"], input=stdin_data, capture_output=True, text=True, check=False)
    assert res.returncode == 1
