...
def test_valid_commit_messages_pass(tmp_path, msg):
    ...
    res = subprocess.run(["bash", str(HOOK_PATH), str(msg_file)], capture_output=True, text=True, check=False)
...
def test_missing_commit_msg_file_fails():
    res = subprocess.run(["bash", str(HOOK_PATH), "/path/does/not/exist"], capture_output=True, text=True, check=False)
    assert res.returncode == 1
